...
//                                Collectors.toList());
        ...
        // TODO: cant find perfect solutions, need to rate all preferences, get top 5
        Queue<TimeRangeEvaluator> q = TimeRangeEvaluator.getTopPreferredSlots(intersections, preferences, solutionNum);
        ...
        if (peopleExcluded > 1) {
            ...
            q.forEach(e -> {
                List<String> peopleNotAvailable = tts.stream()
                        .filter(tt -> {
                            return e.getTimeRange().hasOverlapWith(tt.getScheduleList());
                        })
        ...
        // debug
        q.forEach(e -> {
            e.appendNotes(String.valueOf(e.getRating()));
        });
        // debug

        List<TimeRangeEvaluator> res = new ArrayList<>(q);
        res.sort((o1, o2) -> Double.compare(o2.getRating(), o1.getRating()));
